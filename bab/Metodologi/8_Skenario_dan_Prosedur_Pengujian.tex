...
\label{sec:konfigurasi_baseline_fuzzy}

Konfigurasi baseline (S1--S4) menggunakan APFPlanner dengan parameter navigasi yang bersifat tetap. Pada konfigurasi ini, penguatan gaya tolak obstacle dihitung menggunakan \textit{base gain} berbasis nilai traversability termal mentah ($\tau_{raw}$) secara heuristik, yaitu:
 
\begin{equation}
    G_{baseline} = 1{,}0 + (1{,}0 - \tau_{raw})
    \label{eq:base_gain_baseline}
\end{equation}
 
Formulasi tersebut menghasilkan penguatan gaya tolak yang meningkat secara linear keti\-ka traversability menurun, tanpa memanfaatkan informasi traversability LiDAR maupun meka\-nisme adaptasi berbasis logika fuzzy. Selain itu, gaya jaga-jarak obstacle termal ($F_{cat}$) seb\-agaimana dijelaskan pada Subbab~\ref{sec:cat_keepout} tidak diaktifkan pada konfigurasi ini.
 
Sebaliknya, konfigurasi fuzzy (S5--S8) menggunakan pengendali fuzzy Mamdani yang telah dijelaskan pada Subbab~\ref{sec:fuzzy_Aapf}. Nilai traversability efektif $\tau_{eff}$ dan jarak obstacle terdekat $d_{obs}$ digunakan sebagai masukan fuzzy untuk menghasilkan parameter $S$ (speed scale) dan $G$ (repulsive gain) secara adaptif, sehingga penguatan gaya tolak tidak lagi bersifat linear melainkan bergantung pada kombinasi kedua kondisi lingkungan tersebut. Pada konfigurasi ini, komponen $\mathbf{F}_{cat}$ juga diaktifkan untuk memberikan respons reaktif terhadap obstacle termal berprofil rendah.

Perbedaan utama kedua konfigurasi dirangkum pada Tabel~\ref{tab:konfigurasi}.
